...
var isBoomerang = function (points) {
    ...
    const anyEqual = (nested) => {
        if (nested[0][0] === nested[1][0]
            && nested[0][1] === nested[1][1]) {
            return true;
        } // checking nested[0]===nested[1]
        if (nested[0][0] === nested[2][0]
            && nested[0][1] === nested[2][1]) {
            return true;
        } // checking nested[0]===nested[2]
        if (nested[1][0] === nested[2][0]
            && nested[1][1] === nested[2][1]) {
            return true;
        } // checking nested[1]===nested[2]
        return false;
    }
